...
// When started via Portless (`npm run dev:webapp`) the proxy injects PORTLESS_URL — the named
// https://<worktree>.wardley.localhost address. We open the browser there (not Vite's 127.0.0.1 port)
// and echo it as an extra line under Vite's URLs so it is obvious in the console. Unset for plain
// `npm run dev:webapp:plain`, which then keeps its current behaviour (Vite on :5180, no auto-open).
...
export default defineConfig({
  ...
  server: {
    ...
    // Portless proxies under https://<…>.localhost, so the forwarded Host header is a .localhost
    // subdomain Vite would otherwise reject. (Harmless without Portless.)
