...
- **Quality gates:** the unit suite (Vitest), renderer integration tests in a real browser (Vitest
  Browser Mode, Chromium), end-to-end tests that drive the web app (Playwright, Chromium), ESLint +
  type-checking, and a `dependency-cruiser` check that enforces the DOM-free core boundary — all
